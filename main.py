...
for ticker in tickers:
    ...
    for period in periods:
        ...
        if rf_rmse == min(rf_rmse, svr_rmse, xgb_rmse, ensemb_rmse):
            ...
            minError_pred_value = rf_rmse
            best_model = "Random Forest"
        elif svr_rmse == min(rf_rmse, svr_rmse, xgb_rmse, ensemb_rmse):
            minError_pred = svr_prediction
            minError_pred_value = svr_rmse
            best_model = "SVR"
        elif xgb_rmse == min(rf_rmse, svr_rmse, xgb_rmse, ensemb_rmse):
            minError_pred = xgb_prediction
            minError_pred_value = xgb_rmse
            best_model = "XGB"
        else:
            minError_pred = ensemble_prediction
            minError_pred_value = ensemb_rmse
            best_model = "Ensemble"

        if minError_pred_value < model_error[0][1]:
            model_error[0][3] = period
            model_error[0][2] = y_test
            model_error[0][1] = minError_pred_value
            model_error[0][0] = minError_pred
            model_error[0][4] = best_model
